...
const SearchResultList2 = ({
  ...
  const fetchNextSearchResults = async () => {
    ...
    setError(null);
    try {
      const response = await fetch(`/api/searchByToken?pageToken=${nextPageToken}`);
      ...
      try {
        ...
        const updatedData = await Promise.all(
          data.map(async (clip) => {
            ...
            let videoDetail = clip.videoDetail || videoDetailCacheRef.current.get(videoId) || null;

            if (!videoDetail) {
              const response = await fetch(`/api/getVideo?videoId=${videoId}${indexParam}`);
              if (!response.ok) throw new Error("Network response was not ok");
              videoDetail = await response.json();
              videoDetailCacheRef.current.set(videoId, videoDetail);
            }
  ...
  const handleDownloadClip = async (clip, index, overrideRange) => {
    ...
    try {
      ...
      if (clip.videoDetail?.hls?.video_url) {
        try {
          const { processVideoClip } = await import("@/utils/videoProcessor");
    ...
    const processSearchResults = async () => {
      if (!searchResultData) {
        if (!cancelled) {
          ...
          videoDetailCacheRef.current.clear();
          setNextPageLoading(false);
          setError(null);
        }
        return;
