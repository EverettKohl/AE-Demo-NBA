...
const QuickEdit3Page = () => {
  ...
  const [statusMessage, setStatusMessage] = useState("Select a song format and run Quick Edit 3.");
  ...
  const clipMap = useMemo(() => {
    ...
    try {
      ...
      console.warn("[QuickEdit3] Failed to build clip map", err);
      return null;
    }
  }, [plan]);
  ...
  const runQuickEdit = useCallback(
    async ({ editedSegments = null } = {}) => {
      ...
      setStatusMessage(editedSegments ? "Re-rendering Quick Edit 3 with edits…" : "Running Quick Edit 3…");
      setRunning(true);
      setStageResults(
        STAGE_ORDER.map((stage) => ({
          ...stage,
      ...
      try {
        const res = await fetch("/api/quick-edit-3", {
          method: "POST",
        ...
        const payload = await res.json();
        if (!res.ok) {
          throw new Error(payload.error || "Quick Edit 3 failed");
        }
        ...
        setStatusMessage("Quick Edit 3 failed. Resolve errors and retry.");
        setError(err.message || "Quick Edit 3 failed");
    ...
    <div className="min-h-screen bg-gradient-to-b from-black via-slate-950 to-black text-white">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
        <header className="space-y-3">
          <p className="text-sm uppercase tracking-[0.35em] text-emerald-300">Quick Edit 3</p>
