...
import { fromGenerateEditPlan } from "@/lib/clipMap/adapters";
...
const GenerateEditPage = () => {
  ...
  const [statusMessage, setStatusMessage] = useState("Select a song format and run Generate Edit.");
  ...
  const clipMap = useMemo(() => {
    if (!plan) return null;
    try {
      return fromGenerateEditPlan(plan);
    } catch (err) {
      // eslint-disable-next-line no-console
      console.warn("[GenerateEdit] Failed to build clip map", err);
      return null;
    }
  }, [plan]);
  ...
  const runGenerateEdit = useCallback(
    async ({ editedSegments = null } = {}) => {
      ...
      setJobId(null);
      setStatusMessage(editedSegments ? "Re-rendering Generate Edit with edits…" : "Running Generate Edit…");
      setRunning(true);
      setStageResults(
        STAGE_ORDER.map((stage) => ({
          ...stage,
      ...
      try {
        ...
        const nextBase = payload.videoDataUrl || null;
        ...
        setStatusMessage("Generate Edit failed. Resolve errors and retry.");
        setError(err.message || "Generate Edit failed");
    ...
    [chronologicalOrder, selectedSong, captionsAvailable, includeCaptions]
  );

  const handleRun = useCallback(() => {
    setOverrides({});
    return runGenerateEdit({ editedSegments: null });
  }, [runGenerateEdit]);
  ...
  const handleRerenderWithEdits = useCallback(() => {
    ...
    return runGenerateEdit({ editedSegments });
  }, [buildEditedSegments, runGenerateEdit]);
    ...
    <div className="min-h-screen bg-gradient-to-b from-black via-slate-950 to-black text-white">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-10 space-y-8">
        <header className="space-y-3">
          <p className="text-sm uppercase tracking-[0.35em] text-emerald-300">Generate Edit</p>
            ...
            <div className="rounded-2xl border border-white/10 bg-white/5 p-5 space-y-4 shadow-[0_10px_50px_rgba(0,0,0,0.35)]">
              ...
              {jobId && (
                <div className="space-y-2 rounded-xl border border-white/10 bg-white/5 p-3">
                  ...
                  <a
                    href={`/editor3?geImport=${encodeURIComponent(jobId)}`}
                    className="block w-full rounded-lg bg-emerald-500 text-black text-center font-semibold py-2 hover:bg-emerald-400"
                  >
                    Open in editor
                  </a>
                  <p className="text-xs text-white/60 break-all">/editor3?geImport={encodeURIComponent(jobId)}</p>
                </div>
              )}
            </div>
          </div>
            ...
            {clipMap && (
              <div className="rounded-2xl border border-white/10 bg-white/5 p-4 space-y-4">
                ...
                <ClipMapViewer
                  ...
                  reselectEndpoint="/api/generate-edit"
                />
              </div>
            )}
          </div>
