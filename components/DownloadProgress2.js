...
const DownloadProgressItem = ({ download, onRemove }) => {
  ...
  return (
    <div className={clsx("bg-gray-900/95 border rounded-lg shadow-2xl p-4 backdrop-blur-md animate-in slide-in-from-bottom-2", status === "error" ? "border-red-500/50" : "border-gray-700")}>
          ...
          {status === "downloading" && (
            <>
              <div className="w-full bg-gray-800 rounded-full h-2.5 mb-2 overflow-hidden">
                <div className={clsx("h-full rounded-full transition-all duration-200 ease-out", getStatusColor())} style={{ width: `${progress}%` }} />
              </div>
