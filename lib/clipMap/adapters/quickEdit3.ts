...
export const fromQuickEdit3Plan = (plan: any): ClipMap => {
  ...
  const slots: ClipSlot[] = segments.map((segment, idx) => {
    ...
    // Rapid segments are treated as pauseMusic slots (not beat-locked).
    // This matches song-edit render behavior for rapid clips and allows free duration edits.
  ...
  return {
    id: `quickEdit3:${plan?.songSlug || "unknown"}`,
    mode: "song",
    fps,
    chronologicalOrder,
    slots,
