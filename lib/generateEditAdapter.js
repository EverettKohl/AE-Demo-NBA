...
export const buildGenerateEditRveProject = ({ plan, jobId = null, songUrl = null }) => {
  ...
  const overlays = [];
  const warnings = [];
  const useLocalClips = Boolean(plan?.meta?.useLocalClips || plan?.useLocalClips);
  ...
  const addOverlay = (base, row, offsetSeconds, idSeed, extras = {}) => {
    ...
    if (useLocalClips && !localSrc) {
      throw new Error("Local clip missing for a segment; run materialize before instant demo.");
    }
