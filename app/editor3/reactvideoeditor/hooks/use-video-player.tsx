...
export const useVideoPlayer = (fps: number = 30, externalPlayerRef?: React.RefObject<PlayerRef>) => {
  ...
  const formatTime = useCallback((frames: number) => {
    const totalSeconds = frameToTime(frames, fps);
    const minutes = Math.floor(totalSeconds / 60);
    const seconds = Math.floor(totalSeconds % 60);
    const frameRemainder = Math.round(timeToFrame(totalSeconds % 1, fps));
    const frames2Digits = Math.max(0, frameRemainder % fps).toString().padStart(2, "0");
  ...
  return {
    isPlaying,
    currentFrame,
    currentTimeSec: frameToTime(currentFrame, fps),
    playerRef,
    togglePlayPause,
    formatTime,
    play,
