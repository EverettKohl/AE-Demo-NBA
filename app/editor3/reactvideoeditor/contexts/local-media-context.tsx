...
export const LocalMediaProvider: React.FC<{ children: React.ReactNode }> = ({
    ...
    const loadMediaFiles = async () => {
      try {
        ...
        for (const item of mediaItems) {
          ...
          // Prefer regenerating fresh blob URLs to avoid stale entries
          if (item.fileBlob) {
            itemPath = URL.createObjectURL(item.fileBlob);
          } else if (item.serverPath?.startsWith("blob:")) {
            // Blob path without stored blob data is stale; try to refresh, otherwise drop
            const refreshed = await refreshMediaBlobUrl(item.id);
            if (refreshed) {
              itemPath = refreshed;
            } else {
              // Remove stale entry from IndexedDB
              await deleteFromIndexDB(item.id);
              continue;
            }
          }
  ...
  const addMediaFile = useCallback(
    async (file: File): Promise<LocalMediaFile | void> => {
      setIsLoading(true);
      try {
        // Client-only upload: we always get back a blob-backed item
        const mediaItem = await uploadMediaFile(file);

        // Always generate a fresh blob URL from stored blob data
        const path =
          mediaItem.fileBlob && typeof window !== "undefined"
            ? URL.createObjectURL(mediaItem.fileBlob)
            : mediaItem.serverPath;
