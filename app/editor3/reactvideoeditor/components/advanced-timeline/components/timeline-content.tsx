          ...
          if (dragData.isNewItem || dragData.type) {
            ...
            if (timelineRect) {
              const trackNodes = Array.from(
                timelineRef?.current?.querySelectorAll('.timeline-tracks-container .track') || []
              ) as HTMLElement[];
              const scrollContainer =
                (timelineRef?.current?.closest('.timeline-tracks-scroll-container') as HTMLElement | null);
              const scrollTop =
                (timelineRef?.current?.closest('.timeline-tracks-scroll-container') as HTMLElement | null)
                  ?.scrollTop ?? 0;
              const scrollLeft = scrollContainer?.scrollLeft ?? 0;
              // boundingClientRect already reflects scroll offset; no extra scroll adjustment needed
              const relativeX = e.clientX - timelineRect.left + scrollLeft;
              const relativeY = e.clientY - timelineRect.top;
              const relativeYWithScroll = relativeY + scrollTop;
              ...
              // Calculate track index via DOM hit-testing; fallback to nearest-center
              let targetTrackIndex = 0;
              let foundTrack = false;
              let nearestDist = Number.POSITIVE_INFINITY;
              for (let i = 0; i < trackNodes.length; i++) {
                const rect = trackNodes[i].getBoundingClientRect();
                // Primary: strict hit test
                if (e.clientY >= rect.top && e.clientY <= rect.bottom) {
                  targetTrackIndex = i;
                  foundTrack = true;
                  break;
                }
                // Secondary: pick the row whose center is closest
              ...
              if (!foundTrack && !trackNodes.length) {
                const adjustedY = Math.max(0, relativeYWithScroll);
        ...
        >
          <div className="timeline-tracks-container">
            ...
            const trackGhostElements = ghostElement?.filter(ghost => {
              const ghostTrackIndex = (ghost as any)?.trackIndex;
              const ghostTop = (ghost as any)?.top;
              if (typeof ghostTrackIndex === 'number') {
                return ghostTrackIndex === index;
              }
              const calculatedIndex = Math.floor((ghostTop * tracks.length) / 100);
