...
import { GripVertical, Trash2, Magnet, VolumeX } from 'lucide-react';
  ...
  return (
    <div 
      ...
      style={{ 
        ...
        minWidth: `${TIMELINE_CONSTANTS.HANDLE_WIDTH}px`,
      }}
    >
      ...
      <div className="flex-1 overflow-y-auto overflow-x-hidden scrollbar-hide track-handles-scroll">
        {tracks.map((track, index) => {
          ...
          const getTrackClasses = () => {
            const baseClasses = "track flex items-center px-1 gap-1 border-border";
          ...
          return (
            ...
            >
              ...
              {/* Drag handle */}
              <div
                className={`flex items-center justify-center w-6 h-6 rounded select-none transition-all duration-150 ${
                  isBeingDragged 
                ...
                title="Reorder track"
              >
                <GripVertical className={`w-3.5 h-3.5 ${isBeingDragged ? 'text-primary' : 'text-muted-foreground'}`} />
              </div>
              ...
              <button
                type="button"
                className={`w-6 h-6 inline-flex items-center justify-center rounded text-muted-foreground hover:bg-muted/60 ${track.magnetic ? 'bg-[hsl(var(--warning)/0.2)] text-warning' : ''}`}
              ...
              {/* Mute icon (shows red with strike when muted) */}
              <button
                type="button"
                className={`relative w-6 h-6 inline-flex items-center justify-center rounded hover:bg-muted/60 ${
                  track.muted ? 'bg-red-100' : 'bg-muted'
                }`}
                onClick={() => onToggleMute?.(track.id)}
                title={track.muted ? 'Unmute track' : 'Mute track'}
              >
                <VolumeX
                  className={`w-3.5 h-3.5 ${track.muted ? 'text-red-600' : 'text-foreground'}`}
                  strokeWidth={track.muted ? 2.5 : 2}
                />
                {track.muted && (
                  <span className="pointer-events-none absolute inset-0 flex items-center justify-center">
                    <span className="block w-4 h-[2px] bg-red-600 rotate-45" />
                  </span>
                )}
              </button>
              ...
              <button
                type="button"
                className="w-6 h-6 inline-flex items-center justify-center rounded text-muted-foreground hover:bg-[hsl(var(--destructive)/0.12)] hover:text-destructive"
