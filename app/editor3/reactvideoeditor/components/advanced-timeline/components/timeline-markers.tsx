...
const calculateOptimalInterval = (totalDuration: number, zoomScale: number): number => {
  ...
  // We want major markers roughly every 60-90 pixels for clearer guidance.
  // When zoomed in, tighten spacing so more marks appear.
  const targetPixelSpacing = Math.max(35, 70 / Math.min(zoomScale, 3));
...
export const TimelineMarkers: React.FC<TimelineMarkersProps> = ({
  ...
  const generateMarkers = () => {
    ...
    const zoomMinorFactor = zoomScale >= 1.5 ? 6 : 5;
    if (interval >= 60) {
      minorInterval = interval / zoomMinorFactor; // For minutes/hours
    } else if (interval >= 10) {
      minorInterval = interval / zoomMinorFactor; // For 10s+
    } else if (interval >= 1) {
      minorInterval = interval / zoomMinorFactor; // For seconds
    } else {
      minorInterval = interval / 2; // For sub-second, show 2 minor ticks
    }

    const tickColor = 'var(--timeline-tick, #9ca3af)';
    const tickShadow = '0 0 3px rgba(0,0,0,0.6)';
    ...
    for (let time = 0; time <= totalDuration; time += interval) {
      ...
      markers.push(
        ...
        >
          ...
          />
          <span className="text-xs mt-1 select-none whitespace-nowrap text-muted-foreground">
