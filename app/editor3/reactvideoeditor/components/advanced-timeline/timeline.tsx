...
import { clearTimelineMarkerPosition, findGapsInTrack } from './utils';
...
export const Timeline = forwardRef<TimelineRef, TimelineProps>(({ 
  ...
  const handleStepFrame = useCallback(
    (direction: -1 | 1) => {
      const maxFrame = Math.max(0, Math.round(compositionDuration * fps) - 1);
      const nextFrame = Math.min(Math.max(currentFrame + direction, 0), maxFrame);
    ...
    [compositionDuration, currentFrame, fps, isPlaying, onFrameChange, onPause]
  );
  ...
  // Debug view for visualizing gaps between items per track
  const [showGapDebug, setShowGapDebug] = useState(false);
  const gapsPerTrack = useMemo(() => tracks.map((track, index) => ({
    id: track.id,
    name: track.name || `Track ${index + 1}`,
    gaps: findGapsInTrack(track.items),
  })), [tracks]);
  const tracksWithGaps = useMemo(() => gapsPerTrack.filter(track => track.gaps.length > 0).length, [gapsPerTrack]);
  ...
  // Ensure we start scrolled to the top so all tracks are visible on load
  useEffect(() => {
    scrollToTop();
  }, [scrollToTop, tracks.length]);

  // Add wheel zoom event listener
  useEffect(() => {
    const element = timelineRef?.current?.parentElement; // listen on scroll container
    if (!element) return;
