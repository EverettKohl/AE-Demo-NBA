...
export const findGapsInTrack = (
  ...
  const usableItems = trackItems.filter((item) => !(item as any)?.data?.isGraceBlock);
  if (usableItems.length === 0) return [];

  // Sort items by start time
  const sortedItems = [...usableItems].sort((a, b) => a.start - b.start);
  const gaps: { start: number; end: number }[] = [];

  // Check for gap at the beginning (from grace to first item)
  const firstItem = sortedItems[0];
  if (firstItem.start > TIMELINE_GRACE_SECONDS) {
    gaps.push({ start: TIMELINE_GRACE_SECONDS, end: firstItem.start });
  }
...
export const closeGapsInTrack = (
  ...
  const graceItems = trackItems.filter((item) => (item as any)?.data?.isGraceBlock);
  const usableItems = trackItems.filter((item) => !(item as any)?.data?.isGraceBlock);
  if (usableItems.length === 0) return graceItems;

  // Sort items by start time
  const sortedItems = [...usableItems].sort((a, b) => a.start - b.start);
  const updatedItems: TimelineItem[] = [];
  
  let currentPosition = TIMELINE_GRACE_SECONDS;
  ...
  return [...graceItems, ...updatedItems];
}; 
...
export const calculateMagneticDropPosition = (
  ...
  const usableItems = trackItems.filter((item) => !(item as any)?.data?.isGraceBlock);
  if (usableItems.length === 0) return TIMELINE_GRACE_SECONDS;
...
export const calculateMagneticInsertionPreview = (
  ...
  if (usableItems.length === 0) {
    return {
      insertionIndex: 0,
      insertionStart: TIMELINE_GRACE_SECONDS,
      previewItems: []
    };
  }
  ...
  let currentPos = TIMELINE_GRACE_SECONDS;
...
export const pushItemsDuringResize = (
  ...
  const resizedItem = usableItems.find(item => item.id === resizedItemId);
  if (!resizedItem) {
    return {
      items: [...graceItems, ...usableItems],
      actualStart: newStart,
      actualEnd: newEnd
    };
  }
  ...
  let actualStart = Math.max(TIMELINE_GRACE_SECONDS, newStart);
  ...
  if (expandingLeft) {
    ...
    const blockingItems = usableItems.filter(item => 
  ...
  const updatedItems = usableItems.map(item => {
