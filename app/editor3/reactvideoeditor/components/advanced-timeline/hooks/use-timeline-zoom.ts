...
export const useTimelineZoom = (
  ...
  const handleWheelZoom = useCallback(
    (event: WheelEvent) => {
      ...
      // Fast but usable: scale wheel delta into a larger step with a ramp.
      const raw = Math.abs(event.deltaY);
      const normalized = Math.min(6, raw / 40); // modest ramp for big scrolls
      const speedBoost = 6; // overall speed multiplier (higher = faster)
      const delta =
        -Math.sign(event.deltaY) *
        ZOOM_CONSTRAINTS.wheelStep *
        (1 + normalized) *
        speedBoost;
