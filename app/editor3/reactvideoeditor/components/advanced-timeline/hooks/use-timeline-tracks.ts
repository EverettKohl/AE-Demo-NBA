...
export const useTimelineTracks = ({ 
  ...
  const removeEmptyTracks = useCallback((tracks: TimelineTrack[], shouldRemove: boolean = isAutoRemoveEnabled): TimelineTrack[] => {
    ...
    // Always keep at least one track, even if empty (ignore grace blocks)
    const filteredTracks = tracks.filter(track => 
      track.items.some((item) => !(item as any)?.data?.isGraceBlock)
    );
  ...
  const addNewItem = useCallback((itemData: {
    ...
    try {
      ...
      const leadShift = Math.max(0, TIMELINE_GRACE_SECONDS - newItem.start);
      if (leadShift > 0) {
        newItem = {
          ...newItem,
          start: newItem.start + leadShift,
          end: newItem.end + leadShift,
        };
      }
