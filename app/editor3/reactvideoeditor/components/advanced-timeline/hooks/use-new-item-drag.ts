...
export const useNewItemDrag = ({
  ...
  const handleNewItemDragOver = useCallback(
    (e: React.DragEvent) => {
      ...
      const rawLeftPercentage = Math.max(
        0,
      ...
      const minLeftPercent = totalDuration > 0 ? (TIMELINE_GRACE_SECONDS / totalDuration) * 100 : 0;
      const leftPercentage = Math.max(minLeftPercent, rawLeftPercentage);
  ...
  const handleNewItemDrop = useCallback(
    (
      ...
      const safeStartTime = Math.max(TIMELINE_GRACE_SECONDS, startTime);

      const hasOverlap = checkForOverlap(tracks, trackIndex, safeStartTime, duration);
      ...
      if (onNewItemDrop) {
        onNewItemDrop(itemType, trackIndex, safeStartTime, itemData);
      }
