...
interface UseTimelineHandlersProps {
  ...
  transformTracksToOverlays: ReturnType<typeof useTimelineTransforms>["transformTracksToOverlays"];
  transformOverlaysToTracks: ReturnType<typeof useTimelineTransforms>["transformOverlaysToTracks"];
  fps: number;
}
...
export const useTimelineHandlers = ({
  ...
  const applyTrackUpdate = React.useCallback((updater: (tracks: TimelineTrack[]) => TimelineTrack[]) => {
    isUpdatingFromTimelineRef.current = true;

    setTimelineTracks((prevTracks) => {
      const nextTracks = updater(prevTracks);
      setOverlays(transformTracksToOverlays(nextTracks));
      return nextTracks;
    });
  ...
  const handleSplitItems = React.useCallback((itemId: string, splitTime: number) => {
    const overlayId = parseInt(itemId, 10);
    const splitFrame = timeToFrame(splitTime, fps);
  ...
  const handleItemMove = React.useCallback((itemId: string, newStart: number, newEnd: number, newTrackId: string) => {
    ...
    applyTrackUpdate((prevTracks) => {
      let movedItem: any = null;
      const strippedTracks = prevTracks.map((track) => {
        const remainingItems = track.items.filter((item) => {
          const isMatch = item.id === itemId;
          if (isMatch) {
            movedItem = { ...item, start: newStart, end: newEnd, trackId: newTrackId };
          }
          return !isMatch;
        });
        return { ...track, items: remainingItems };
      });

      if (!movedItem) {
        console.error(`[TIMELINE-HANDLERS][${timestamp}] ❌ Timeline item not found for move`, { itemId, newTrackId });
        return prevTracks;
      }

      const updatedTracks = strippedTracks.map((track) => {
        if (track.id === newTrackId) {
          const items = [...track.items, movedItem].sort((a, b) => a.start - b.start);
          return { ...track, items };
        }
        return track;
      });
  ...
  }, [overlays, applyTrackUpdate, isUpdatingFromTimelineRef]);
  ...
  const handleItemResize = React.useCallback((itemId: string, newStart: number, newEnd: number) => {
    ...
    applyTrackUpdate((prevTracks) => prevTracks.map((track) => ({
      ...track,
      items: track.items.map((item) => item.id === itemId ? { ...item, start: newStart, end: newEnd } : item),
    })));
  }, [applyTrackUpdate, isUpdatingFromTimelineRef]);
    ...
    try {
      ...
      if (itemType === 'video' && itemData?.data) {
        ...
        if (video._isLocalMedia && video.src) {
          ...
          // External media - use adaptor; fall back to embedded URLs when adaptor is missing
          const adaptor = videoAdaptors.find((a) => a.name === video._source);
          videoUrl =
            adaptor?.getVideoUrl(video, "hd") ||
            (video.videoFiles?.[0]?.url as string | undefined) ||
            (video.src as string | undefined) ||
            "";
        }

        if (!videoUrl) {
          console.warn("[TIMELINE-HANDLERS] Missing video src for dropped item", { video });
          return;
        }
        ...
        const durationInFrames = timeToFrame(durationFromMetadata, fps);
        ...
        newOverlay = {
          ...
          height,
          durationInFrames,
          from: timeToFrame(startTime, fps),
          rotation: 0,
          row: trackIndex,
          isDragging: false,
        ...
        const defaultImageDurationFrames = timeToFrame(5, fps);
        ...
        newOverlay = {
          ...
          width,
          height,
          durationInFrames: defaultImageDurationFrames, // 5 seconds default
          from: timeToFrame(startTime, fps),
          rotation: 0,
          row: trackIndex,
          isDragging: false,
      ...
      setTimelineTracks(transformOverlaysToTracks(updatedOverlays));
      setTimeout(() => {
        isUpdatingFromTimelineRef.current = false;
      }, 500);
  ...
  }, [overlays, setOverlays, setSelectedOverlayId, setActivePanel, videoAdaptors, imageAdaptors, getAspectRatioDimensions, fps, setTimelineTracks, transformOverlaysToTracks]);
