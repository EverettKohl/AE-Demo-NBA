...
export const MediaOverlayPanel = <T extends MediaItem>({
  ...
  const [activeTab, setActiveTab] = useState<string>("twelve-labs-clips");

  // Filter items based on active tab (only Twelve Labs)
  const filteredItems = items.filter(item => item._source === activeTab);
    ...
    <div className="flex flex-col p-2 h-full overflow-hidden">
      {!isEditMode ? (
        <>
          ...
          {/* Source Tabs (only Twelve Labs) */}
          {sourceResults.length > 0 && (
            <div className="shrink-0 mb-4">
              <UnifiedTabs
                sourceResults={sourceResults.filter((s) => s.adaptorName === "twelve-labs-clips")}
                activeTab={activeTab}
