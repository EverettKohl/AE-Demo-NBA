...
export const uploadMediaFile = async (file: File): Promise<UserMediaItem> => {
  try {
    ...
    // Client-only: always store as blob in IndexedDB
...
async function uploadAsBlob(
  ...
  const mediaItem: UserMediaItem = {
    ...
    name: file.name,
    type: fileType,
    // Do not rely on a single blob URL string; it is origin-bound and short-lived.
    // Store the blob itself and regenerate URLs when needed.
    serverPath: "", 
    size: file.size,
