...
const FORMATS_DIR = path.join(process.cwd(), "data", "song-formats-6");
const SONGS_DIR = path.join(process.cwd(), "public", "songs");
const PREVIEWS_DIR = path.join(process.cwd(), "public", "previews", "format-builder-6");
...
export async function POST(request) {
  ...
  const outputPath = path.join(PREVIEWS_DIR, outputFile);
    ...
    return NextResponse.json({
      success: true,
      url: `/previews/format-builder-6/${path.basename(outputPath)}`,
      path: outputPath,
    });
  } catch (error) {
