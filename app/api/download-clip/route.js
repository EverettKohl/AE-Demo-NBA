...
export async function GET(req) {
  ...
  try {
    const videoResponse = await fetch(`${apiUrl}/indexes/${indexId}/videos/${videoId}`, {
        ...
        ffmpeg.on("error", (error) => {
          if (error.code === "ENOENT") {
            ...
            if (isVercel) {
              reject(
                new Error(
                  "Video clip download is not available on this deployment platform. Please use a platform that supports ffmpeg."
                )
              );
            } else {
