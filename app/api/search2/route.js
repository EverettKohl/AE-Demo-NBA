...
import { getClipUrl } from "@/utils/cloudinary2";
import { resolveKillBillVideoIds, listKillBillVideoIds } from "@/lib/twelveLabs2/videoCatalog";
import { isBannedClip, getCloudinaryId, getCanonicalVideoId } from "@/lib/killBillAgent2/utils";
import { searchTwelveLabsClips } from "@/lib/twelveLabs2/searchClient";
...
export async function POST(request) {
  try {
    ...
    if (!resolvedVideoIds.length) resolvedVideoIds = listKillBillVideoIds();
    ...
    const searchResults = await searchTwelveLabsClips({ query, videoIds: resolvedVideoIds, limit: limit || 12 });
    ...
    const results = searchResults
      .map((item) => {
        ...
        try {
          ...
          console.error("Error generating clip URL for result:", err);
          return null;
        }
      })
      .filter(Boolean);

    return NextResponse.json({ results, pageInfo: { total_results: results.length } });
  } catch (error) {
