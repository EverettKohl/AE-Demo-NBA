...
// Registers debug render hook expected by quick-edit-6
import "../song-edit/route.js";
// Note: the original route registered other handlers for side effects; omitted here.
import { loadInstantClipPool } from "../../../lib/songEdit.js";
import {
  loadQuickEdit6Format,
  buildQuickEdit6Segments,
  assignQuickEdit6Clips,
  reselectQuickEdit6Clip,
  trimQuickEdit6Segments,
  createClipPoolSummary,
} from "../../../lib/quickEdit6.js";
...
function buildDrawtextFilters({ captions, width, height }) {
  ...
  const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "qe6-captions-"));
  ...
  words.forEach((word) => {
    ...
    if (style.spacing) draw.push(style.spacing.replace(":", ""));
    if (rawText === "TESTIFY") {
      console.log("[quick-edit-6] TESTIFY drawtext:", draw[0]);
    }
    pushFilter(style.mode || "default", draw.join(":"), start, end);
  });
...
export async function POST(request) {
  ...
  try {
    ...
    if (reselect && typeof reselect === "object") {
      ...
      format = loadQuickEdit6Format(songSlug);
      segmentsPayload = buildQuickEdit6Segments(format);
      ...
      const replacement = reselectQuickEdit6Clip({
        segmentIndex,
        segment,
        pool,
        usedPoolIndices,
    ...
    format = await runStage(
      "loadFormat",
      STAGE_ORDER[0].label,
      () => ({ result: loadQuickEdit6Format(songSlug), stageMessage: `Loaded format "${songSlug}"` }),
      stageResults
    );
    ...
    segmentsPayload = await runStage(
      ...
      STAGE_ORDER[1].label,
      () => {
        const segmentsData = buildQuickEdit6Segments(format);
        return {
          result: segmentsData,
    ...
    assignPayload = await runStage(
      ...
      STAGE_ORDER[2].label,
      () => {
        const assignResult = assignQuickEdit6Clips({
    ...
    const trimOutcome = await runStage(
      ...
      STAGE_ORDER[3].label,
      () => {
        const result = trimQuickEdit6Segments({ segments: segmentsPayload.segments, fps: segmentsPayload.fps });
    ...
    if (includeCaptions && format?.captions && renderResult?.dataUrl) {
      try {
        const tempDir = fs.mkdtempSync(path.join(os.tmpdir(), "qe6-caption-overlay-"));
        const baseAbsPath = path.join(tempDir, "base.mp4");
        writeDataUrlToFile(renderResult.dataUrl, baseAbsPath);

        ensureDir(QE6_PREVIEWS_DIR);
        const captionedFileName = `${songSlug}-${timestampId()}-captioned.mp4`;
        const captionedAbsPath = path.join(QE6_PREVIEWS_DIR, captionedFileName);
        await renderCaptionsOnMp4({
          inputPath: baseAbsPath,
          captions: format.captions,
          outputPath: captionedAbsPath,
        });
        captionedResult = { url: `/previews/quick-edit-6/${captionedFileName}` };
      } catch (err) {
        console.warn("[quick-edit-6] caption overlay failed:", err?.message || err);
      }
    }
  } catch (error) {
    return NextResponse.json(
      {
        error: error?.message || "Quick Edit 6 failed",
        stageResults,
      },
      { status: 500 }
    );
  ...
  try {
    if (renderResult?.dataUrl) {
      ensureDir(QE6_PREVIEWS_DIR);
      const baseFileName = `${songSlug}-${jobId}.mp4`;
      const baseAbsPath = path.join(QE6_PREVIEWS_DIR, baseFileName);
      writeDataUrlToFile(renderResult.dataUrl, baseAbsPath);
      basePreviewUrl = `/previews/quick-edit-6/${baseFileName}`;
    }
  } catch (err) {
    console.warn("[quick-edit-6] Failed to persist base mp4", err);
  }

  const finalVideoUrl = captionedResult?.url || basePreviewUrl || null;
  ...
  try {
    if (finalVideoUrl) {
      ...
      try {
        const { buildQuickEdit6RveProject } = await import("../../editor/quickEdit6Adapter.js");
        rveProject = buildQuickEdit6RveProject({
          plan,
          jobId,
          renderUrl: captionedResult?.url || basePreviewUrl || finalVideoUrl,
          songUrl: plan?.songFormat?.source || null,
        });
        // Ensure a sound overlay exists; some earlier payloads lacked audio.
        if (!Array.isArray(rveProject?.overlays) || !rveProject.overlays.some((o) => String(o?.type).toLowerCase() === "sound" || o?.type === 6)) {
          const durationFrames =
            plan?.totalFrames ||
            (rveProject?.overlays || []).reduce(
              (max, o) => Math.max(max, (o.from || 0) + (o.durationInFrames || 0)),
              0
            );
          const songSrc =
          ...
          rveProject.overlays = [
            ...(rveProject?.overlays || []),
            {
              id: (rveProject?.overlays?.length || 0) + 1000,
              type: "sound",
              row: 5,
              from: 0,
              durationInFrames: Math.max(1, durationFrames || 1),
              left: 0,
              top: 0,
              width: 1920,
              height: 100,
              rotation: 0,
              isDragging: false,
              content: plan?.songSlug || "Song",
              src: songSrc,
              startFromSound: 0,
              videoDurationInFrames: Math.max(1, durationFrames || 1),
              mediaSrcDuration: plan?.songFormat?.meta?.durationSeconds || null,
              styles: {
                opacity: 1,
                volume: 1,
              },
        ...
        console.warn("[quick-edit-6] Failed to build rve project for import", err);
      }
      const payload = {
        jobId,
    ...
    console.warn("[quick-edit-6] Failed to write editor import", err);
  }
