...
export const useTimelineHandlers = ({
  ...
  const handleItemMove = React.useCallback((itemId: string, newStart: number, newEnd: number, newTrackId: string) => {
    ...
    if (overlay) {
      ...
      const durationSeconds = Math.max(0, newEnd - newStart);
      const clampedStartTime = Math.max(MIN_START_TIME_SECONDS, newStart);
      const clampedEndTime = clampedStartTime + durationSeconds;
      
      const updatedOverlay: Overlay = {
        ...overlay,
        from: Math.round(clampedStartTime * FPS),
        durationInFrames: Math.round((clampedEndTime - clampedStartTime) * FPS),
        row: newRow,
      };
