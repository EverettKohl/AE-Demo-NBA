...
const SoundsOverlayPanel: React.FC = () => {
  ...
  const handleAddToTimeline = async (sound: AudioWithSource) => {
    ...
    /**
     * Use a functional updater so we never lose concurrent overlay changes
     * (e.g., autosave restore finishing while the duration request is in-flight).
     * Also make ID generation resilient to non-numeric IDs by coercing to numbers
     * and falling back to timestamps when needed.
     */
    let nextOverlayId = 0;
    setOverlays((prevOverlays) => {
      ...
      const highestId = updatedOverlays.reduce<number>((max, overlay) => {
        const numericId = typeof overlay.id === 'number' ? overlay.id : Number(overlay.id);
        return Number.isFinite(numericId) ? Math.max(max, numericId) : max;
      }, -1);

      nextOverlayId = highestId + 1;
      ...
      const overlayWithId = { ...newSoundOverlay, id: nextOverlayId } as SoundOverlay;
      return [...updatedOverlays, overlayWithId];
    });
