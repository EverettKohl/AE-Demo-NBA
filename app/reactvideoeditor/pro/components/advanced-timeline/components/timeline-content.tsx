          ...
          if (dragData.isNewItem || dragData.type) {
            ...
            if (timelineRect) {
              ...
              const minLeftPercentage = Math.min(
                100,
                (MIN_START_TIME_SECONDS / viewportDuration) * 100
              );
              const clampedLeftPercentage = Math.max(minLeftPercentage, leftPercentage);
              const dropTime = Math.max(
                MIN_START_TIME_SECONDS,
                (clampedLeftPercentage / 100) * viewportDuration
              );
