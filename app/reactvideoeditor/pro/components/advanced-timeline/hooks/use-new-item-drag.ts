...
import { MIN_START_TIME_SECONDS } from '../../../../../constants';
...
export const useNewItemDrag = ({
  ...
  const handleNewItemDragOver = useCallback(
    (e: React.DragEvent) => {
      ...
      // Calculate horizontal position (clamped to minimum start time)
      const leftPercentage = Math.max(
        0,
        Math.min(100, (relativeX / timelineRect.width) * 100)
      );
      const minLeftPercentage = Math.min(
        100,
        (MIN_START_TIME_SECONDS / totalDuration) * 100
      );
      let clampedLeftPercentage = Math.max(minLeftPercentage, leftPercentage);
      ...
      let startTime = (clampedLeftPercentage / 100) * totalDuration;
      if (startTime < MIN_START_TIME_SECONDS) {
        startTime = MIN_START_TIME_SECONDS;
        clampedLeftPercentage = Math.min(
          100,
      ...
      const currentPosition = {
        trackIndex,
        left: Math.round(clampedLeftPercentage),
        isValid: isValidDrop,
      };
  ...
  const handleNewItemDrop = useCallback(
    (
      ...
      const clampedStartTime = Math.max(MIN_START_TIME_SECONDS, startTime);

      const hasOverlap = checkForOverlap(tracks, trackIndex, clampedStartTime, duration);
      ...
      if (onNewItemDrop) {
        onNewItemDrop(itemType, trackIndex, clampedStartTime, itemData);
      }
