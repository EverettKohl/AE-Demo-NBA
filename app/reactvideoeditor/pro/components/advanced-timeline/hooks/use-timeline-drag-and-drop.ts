...
export const useTimelineDragAndDrop = ({
  ...
  const validateDropPosition = useCallback(
    (
      ...
      if (targetStart < MIN_START_TIME_SECONDS) {
        return { isValid: false, reason: `Cannot place item before ${MIN_START_TIME_SECONDS}s` };
      }
  ...
  const handleDrag = useCallback(
    (clientX: number, clientY: number) => {
      ...
      // Ensure boundaries - prevent going before the minimum allowed start time
      newStart = Math.max(MIN_START_TIME_SECONDS, newStart);
      ...
      if (isMultiDrag) {
        ...
        for (const snapshot of dragInfo.selectedItemsSnapshot) {
          ...
          // Check left boundary constraint (prevent going before minimum start time)
          if (itemNewStart < MIN_START_TIME_SECONDS) {
            const leftConstraint = MIN_START_TIME_SECONDS - snapshot.originalStart;
        ...
        for (const snapshot of dragInfo.selectedItemsSnapshot) {
          ...
          if (snapshot.id === dragInfo.id) {
            ...
            itemNewStart = Math.max(MIN_START_TIME_SECONDS, snapshot.originalStart + deltaTime);
        ...
        if (primarySnapshot) {
          ...
          const trackItemsData = sortedTrackGroups.map(([originalTrackIndex, trackItems]) => {
            ...
            const items = trackItems.map(snapshot => {
              const targetStart = Math.max(MIN_START_TIME_SECONDS, snapshot.originalStart + deltaTime);
              return {
                itemId: snapshot.id,
                start: targetStart,
                end: targetStart + snapshot.originalDuration
              };
            });
