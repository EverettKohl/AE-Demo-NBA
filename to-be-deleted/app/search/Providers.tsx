...
export default function Providers({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const isMetaMaskError = (event: ErrorEvent | PromiseRejectionEvent) => {
      const message = (event as any)?.message || (event as any)?.reason?.message || (event as any)?.reason;
      const stack = (event as any)?.filename || (event as any)?.reason?.stack;
      return (
