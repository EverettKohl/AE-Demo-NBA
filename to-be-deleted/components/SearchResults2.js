...
import SearchResultList from "./SearchResultList2";
import LoadingSpinner from "./LoadingSpinner";
import ErrorFallback from "./ErrorFallback";

const SearchResults2 = ({ updatedSearchData, setUpdatedSearchData, textSearchQuery, textSearchSubmitted, searchMode = "quick" }) => {
  ...
  useEffect(() => {
    if (textSearchSubmitted) {
      queryClient.invalidateQueries(["textSearch", textSearchQuery, searchMode]);
    }
  }, [textSearchSubmitted, textSearchQuery, searchMode, queryClient]);

  const resultCount =
    textSearchResultData?.pageInfo?.total_results ??
    textSearchResultData?.textSearchResults?.length ??
    0;
  const hasResults = textSearchSubmitted && resultCount > 0;
